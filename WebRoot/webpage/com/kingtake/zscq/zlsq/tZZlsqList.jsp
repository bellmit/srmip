...
<div class="easyui-layout" fit="true">
  <div region="center" style="padding: 1px;">
    <t:datagrid name="tZZlsqList" checkbox="true" fitColumns="true" title="专利申请列表" actionUrl="tZZlsqController.do?datagrid&role=fmr" idField="id" fit="true" queryMode="group" onDblClick="goViewDetail">
      ...
      <t:dgDelOpt exp="apprStatus#eq#0" title="删除" url="tZZlsqController.do?doDel&id={id}" />
      <t:dgToolBar title="录入" icon="icon-add" url="tZZlsqController.do?goUpdate" funname="add" height="100%"></t:dgToolBar>
      <t:dgFunOpt exp="apprStatus#eq#0" title="编辑" funname="goUpdate(id)"></t:dgFunOpt>
      <t:dgFunOpt exp="apprStatus#eq#3" title="编辑" funname="goUpdate(id)"></t:dgFunOpt>
      <t:dgToolBar title="查看" icon="icon-search" url="tZZlsqController.do?goUpdate&opt=view" funname="detail" height="100%"></t:dgToolBar>
      <t:dgFunOpt title="发送审批" funname="sendTravelAppr(id, apprStatus)"></t:dgFunOpt>
...
function sendTravelAppr(id, apprStatus){
	...
	sendApprDialog(title, url, width, height, id,apprStatus,'<%=ApprovalConstant.APPR_TYPE_ZLSQ%>');
    }
    ...
    function openProcess(id){
        addTab("专利申请管理", "tZZlsqController.do?goFmrMain&id="+id, "default");
    }
