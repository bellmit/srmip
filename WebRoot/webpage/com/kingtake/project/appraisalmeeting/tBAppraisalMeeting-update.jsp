...
%>
  <c:if test="${!empty tBAppraisalMeetingPage.msgText and tBAppraisalMeetingPage.meetingStatus eq rebut }">
    ...
    <textarea id="msgTextArea" rows="5" cols="5" style="width:100%;height:100%;display: none;" >${tBAppraisalMeetingPage.msgText}</textarea>
  </c:if>
  <c:if test="${tBAppraisalMeetingPage.meetingStatus eq send }"><div align="center"><img alt="已提交" src="plug-in/easyui/themes/icons/ok.png"><span class="meetingTip">已提交</span></div></c:if>
  <c:if test="${tBAppraisalMeetingPage.meetingStatus eq finish }"><div align="center"><img alt="已通过" src="plug-in/easyui/themes/icons/ok.png"><span class="meetingTip">已通过</span></div></c:if>
  <div>
  <t:formvalid formid="formobj" dialog="true"  layout="table" action="tBAppraisalMeetingController.do?doUpdate" callback="@Override saveCallback" tiptype="1" postonce="false" btnsub="saveBtn">
  ...
  <c:if test="${empty tBAppraisalMeetingPage.id or tBAppraisalMeetingPage.meetingStatus eq unsend or tBAppraisalMeetingPage.meetingStatus eq rebut}">
    <!-- <input id="saveBtn" class="Button" value="保存" type="button"> -->
    <a class="easyui-linkbutton" id="saveBtn">保存</a>
    </c:if>
    <c:if test="${!empty tBAppraisalMeetingPage.id and tBAppraisalMeetingPage.meetingStatus eq unsend or tBAppraisalMeetingPage.meetingStatus eq rebut}">
    <!-- <input id="subBtn" class="Button" value="提交" type="button" onclick="submitProcess()" > -->
    <a class="easyui-linkbutton" id="subBtn" onclick="submitProcess()">提交</a>
    </c:if>
    </div>
  </t:formvalid>
  </div>
	            ...
	            formatter: function(value,row,index){
	            	...
	            	if(row.memberList.length>0){
		            	for(var i=0;i<row.memberList.length;i++){
		            		...
		            		if(row.memberList[i].FLAG =="1"){//新增人员名字颜色区分
		            			result[i]="<font color='green'>"+row.memberList[i].MEMBER_NAME+"</font><span class='deleteMember' onclick=\"deleteMember('"+row.id+"','"+row.memberList[i].ID+"')\" title=\"移除该参会人员\"><img class='delimg' src=\"webpage/com/kingtake/project/appraisalmeeting/cancel.png\"></span>"
		            		}else{
		            			result[i]=row.memberList[i].MEMBER_NAME+"<span class='deleteMember' onclick=\"deleteMember('"+row.id+"','"+row.memberList[i].ID+"')\" title=\"移除该参会人员\"><img class='delimg' src=\"webpage/com/kingtake/project/appraisalmeeting/cancel.png\"></span>"
		            		}
		  			      }
		            	}
		            }
	            ...
	            formatter: function(value,row,index){
	            	...
	            	if(row.memberList.length>0){
		            	for(var i=0;i<row.memberList.length;i++){
		            		...
		            		if(row.memberList[i].FLAG =="1"){//新增人员名字颜色区分
		            			result[i]="<span class=\"deleteMember\" onclick=deleteMember('"+row.id+"','"+row.memberList[i].ID+"') title=\"移除该参会人员\"><font color='green'>"+row.memberList[i].MEMBER_NAME+"</font><img class='delimg' src=\"webpage/com/kingtake/project/appraisalmeeting/cancel.png\"></span>"
		            		}else{
		            			result[i]="<span class=\"deleteMember\" onclick=deleteMember('"+row.id+"','"+row.memberList[i].ID+"') title=\"移除该参会人员\">"+row.memberList[i].MEMBER_NAME+"<img class='delimg' src=\"webpage/com/kingtake/project/appraisalmeeting/cancel.png\"></span>"
		            		}
