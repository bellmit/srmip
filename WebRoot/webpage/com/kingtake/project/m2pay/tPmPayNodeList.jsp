...
<div class="easyui-layout" fit="false" style="height: 400px;">
  <div region="center" style="padding: 1px;">
    <t:datagrid name="tPmPayNodeList" checkbox="true" fitColumns="true" actionUrl="tPmPayNodeController.do?datagrid&tpId=${projectId}" idField="id" fit="true" queryMode="group" sortName="createDate"
      sortOrder="desc" onDblClick="dblDetail">
      ...
      <t:dgCol title="支付条件" field="payCondition" queryMode="group" width="120"></t:dgCol>
      ...
      <t:dgFunOpt exp="auditFlag#eq#0" title="编辑" funname="updatePayNode(id)" />
      <t:dgDelOpt exp="auditFlag#eq#0" title="删除" url="tPmPayNodeController.do?doDel&id={id}" />
      <!-- zhangls 2016-03-18 应胡参谋要求屏蔽录入操作 begin -->
      <%-- <t:dgToolBar title="录入" icon="icon-add" url="tPmPayNodeController.do?goAdd&tpId=${projectId}" funname="add" width="600" height="350"></t:dgToolBar> --%>
      <!-- zhangls 2016-03-18 应胡参谋要求屏蔽录入操作 end -->
      <t:dgToolBar title="查看" icon="icon-search" url="tPmPayNodeController.do?goUpdate" funname="detail" width="600" height="400"></t:dgToolBar>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    //给时间控件加上样式
    $("#tPmPayNodeListtb").find("input[name='payTime_begin']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
      WdatePicker({
        dateFmt : 'yyyy-MM-dd'
      });
    });
    $("#tPmPayNodeListtb").find("input[name='payTime_end']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
      WdatePicker({
        dateFmt : 'yyyy-MM-dd'
      });
    });
    $("#tPmPayNodeListtb").find("input[name='auditTime_begin']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
      WdatePicker({
        dateFmt : 'yyyy-MM-dd'
      });
    });
    $("#tPmPayNodeListtb").find("input[name='auditTime_end']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
      WdatePicker({
        dateFmt : 'yyyy-MM-dd'
      });
    });
  ...
  //导出
  function ExportXls() {
    JeecgExcelExport("tPmPayNodeController.do?exportXls&tpId=" + $("#projectId").val(), "tPmPayNodeList");
  }
