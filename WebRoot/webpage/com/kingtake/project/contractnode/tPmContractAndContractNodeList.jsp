...
var li_east2 = 0;
	...
	function refreshInOrPayNode(rowIndex, rowData){
		...
		if(rowData){
			...
			if( $("#income").val() == "true" ){
				title = '指定来款节点';
				var url = 'tPmIncomeNodeController.do?tPmIncomeNodeAndContractNode&projectId=${projectId}&checkFlag=0';
				$('#inComeNode').panel("refresh", url+"&contractNodeId="+contractNodeId);
// 				var url = $('#inComeNode').attr("href");
// 				refreshIncomeNode(rowIndex, rowData);
			}else{
				title = '指定支付节点';
				var url = 'tPmPayNodeController.do?tPmPayNodeAndContractNode&projectId=${projectId}&checkFlag=0';
