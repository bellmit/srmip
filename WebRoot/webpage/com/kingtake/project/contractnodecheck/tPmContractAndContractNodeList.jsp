...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="tPmContractNodeList" checkbox="false" fitColumns="false" title="合同节点"
  	actionUrl="tPmContractNodeCheckController.do?nodedatagrid&projectId=${projectId}&inOutFlag=${inOutFlag}" 
   ...
   <t:dgCol title="合同名称"  field="contractName"    queryMode="group"  width="120"></t:dgCol>
   ...
   <t:dgCol title="考核状态"  field="checkFlag"  hidden="false" codeDict="1,KHZT" queryMode="group"  width="80"></t:dgCol>
   <t:dgCol title="操作" field="opt" width="80" frozenColumn="true"></t:dgCol>
<%--    <t:dgFunOpt funname="detailContractView(id,inOutFlag)" title="查看合同信息"></t:dgFunOpt> --%>
   <t:dgFunOpt funname="nodeAudit(id)" title="节点考核"></t:dgFunOpt>
    <t:dgToolBar title="查看合同信息" icon="icon-search"   funname="detailContractView"
   	 	height="600" width="725"></t:dgToolBar>
...
<!-- <div style="width:310px; overflowhidden;" id="eastPanel" data-options="region:'east', split:true"> -->
<%--    <c:if test="${income}"> --%>
<!--    	 <div class="easyui-panel" style="padding: 1px;" fit="true" border="false"  -->
<%--     	href="tPmIncomeNodeController.do?tPmIncomeNodeAndContractNode&projectId=${projectId}"></div> --%>
<%--    </c:if> --%>
<%--    <c:if test="${!income}"> --%>
<!--    	 <div class="easyui-panel" style="padding: 1px;" fit="true" border="false"  -->
<%--     	href="tPmPayNodeController.do?tPmPayNodeAndContractNode&projectId=${projectId}"></div> --%>
<%--    </c:if> --%>
<!-- </div> -->
</div>
...
<script type="text/javascript">
	...
	function detailContractView(title, url, id, width, height){
		...
		if(rowsData[0].inOutFlag=='i'){
			url='tPmIncomeContractApprController.do?goUpdateTab&load=detail&node=false&receive=false&contractId='+ rowsData[0].inOutContractid;
		}else{
			url='tPmOutcomeContractApprController.do?goUpdateTab&load=detail&node=false&receive=false&contractId='+ rowsData[0].inOutContractid;
		}
		createdetailwindow('合同信息',url,width, height);
	}
	
	function nodeAudit(id){
// 		add('合同节点考核','tPmContractNodeCheckController.do?goAdd&contractNodeId='+id);
			$.dialog({
				content: 'url:tPmContractNodeCheckController.do?goAdd&contractNodeId='+id,
				lock : true,
				//zIndex:1990,
				width:650,
				height:520,
...
//		 					add("发送","tOReceiveBillController.do?goSend&id="+id,"",520,220);
							...
							$.post('tPmContractNodeCheckController.do?doAdd',addForm,function(data){
								data = JSON.parse(data);
// 								iframe.uploadFile(data);
								var id = data.obj.id;
								console.log(frameElement.window);
								$.dialog({
									content: 'url:tPmContractNodeCheckController.do?goSend&id='+id,
									lock : true,
									//zIndex:1990,
									width:520,
									height:220,
