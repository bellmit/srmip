...
<c:if test="${planContractFlag eq 1 || stage eq 'ys'}">
<t:base type="jquery,easyui,tools"></t:base>
</c:if>
<c:if test="${schoolCooperationFlag eq 1 }">
<t:base type="jquery,easyui,tools"></t:base>
</c:if>
...
<div class="easyui-layout" fit="true" style="height: 500px;">
  <div region="center" style="padding: 1px;">
    <t:datagrid name="tPmIncomeApplyList" onDblClick="goDetail" fitColumns="true" title="到账信息查询及认领" actionUrl="tPmIncomeApplyController.do?datagrid&project.id=${projectId}&stage=${stage}" idField="id" fit="true"
      queryMode="group">
      <t:dgCol title="主键" field="id" hidden="true" queryMode="group" width="120"></t:dgCol>
      <t:dgCol title="关联项目id" field="project.Id" hidden="true" queryMode="group" width="120"></t:dgCol>
      <t:dgCol title="年度" field="applyYear" queryMode="single" width="80"></t:dgCol>
      <t:dgCol title="到账凭证号" field="voucherNo" queryMode="single" width="100"></t:dgCol>
      <t:dgCol title="发票号" field="invoice_invoiceNum1" queryMode="single" width="100"></t:dgCol>
      <t:dgCol title="认领金额" field="applyAmount" queryMode="single" width="100"></t:dgCol>
      ...
      <t:dgCol title="科研审核状态" field="auditStatus" extendParams="formatter:auditStatusFormat,"  width="100"></t:dgCol>
      <t:dgCol title="财务审核状态" field="cwStatus" replace="未审核_0,通过_1,不通过_2" width="100"></t:dgCol>
      ...
      <c:if test="${stage ne 'ys' }">
      <t:dgCol title="操作" field="opt" width="200" frozenColumn="true"></t:dgCol>
      </c:if>
      <t:dgFunOpt exp="auditStatus#ne#1&&auditStatus#ne#2" funname='goEdit(id)' title="编辑"></t:dgFunOpt>
      <t:dgDelOpt exp="auditStatus#eq#0" title="删除" url="tPmIncomeApplyController.do?doDel&id={id}" />
      <t:dgFunOpt exp="auditStatus#eq#0" title="提交审核" funname="submitAudit(id)" />
      <t:dgFunOpt exp="auditStatus#eq#3" title="提交审核" funname="submitAudit(id)" />
      
      <!--  c:if test="${planContractFlag eq 2 }"> -->
      <c:if test="${1 eq 2 }">
      	<c:if test="${schoolCooperationFlag eq 0 }">
      		<t:dgToolBar title="认款" icon="icon-add" funname="goAdd" ></t:dgToolBar>
      	</c:if>
      </c:if>
      
      <c:if test="${stage eq 'ys' }">
...
<script type="text/javascript">
  ...
  //提交审核
    function submitAudit(id) {
    	$.ajax({
            url:'tPmIncomeApplyController.do?checkInfo',
            type:'POST',
            dataType:'json',
            cache:'false',
            data:{'id':id},
...
//录入预算之前进行判断
  function addFun(title,addurl,gname,width,height){
  	var rowsData = $('#tPmIncomeApplyList').datagrid('getSelections');
  	...
  	addurl += '&incomeApplyId='+rowsData[0].id;
