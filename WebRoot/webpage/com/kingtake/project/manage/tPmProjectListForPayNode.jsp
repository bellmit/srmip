...
<div class="easyui-layout" fit="true" id="main_div">
	<div region="center" style="padding: 1px;">
		<t:datagrid name="tPmProjectList" checkbox="false" fitColumns="false" title="待审项目列表"
			actionUrl="tPmProjectController.do?datagridForProject&key=payNode" onLoadSuccess="loadSuccess" 
			idField="id" fit="true" queryMode="group" onClick="queryPayDetail">
			...
			<t:dgCol title="支付节点状态" field="finishStatus" replace="已审核_1,未审核_0" query="true" queryMode="single" width="120"></t:dgCol>
			
			<t:dgCol title="操作" field="opt" width="300" frozenColumn="true"></t:dgCol>
			<t:dgFunOpt funname="return false;" title="查看支付节点"></t:dgFunOpt>
	...
	<div data-options = 
		...
		"
		style="width: 580px; overflow: hidden;" id="eastPanel">
...
<script type="text/javascript">
    ...
    function queryPayDetail(rowIndex, rowDate){
    	queryPayDetail2(rowDate.id);
    }

	var li_east = 0;
	//查看类型链接
	function queryPayDetail2(id){
	    var title = '支付节点列表';
	    if(li_east == 0){
	        $('#main_div').layout('expand','east');
	    }
	    $('#main_div').layout('panel','east').panel('setTitle', title);
	    $('#userListpanel').panel("refresh", "tPmPayNodeController.do?tPmPayNodeListForDepartment&projectId=" + id);
	}
