(function(window, $) {
    ...
    SideCatalog.prototype = {
        initCatalogView : function() {
            var html, cataItemClass, cataIndxClass, html, heads;
            html = '<div id="sidebar">' + '<div class="sidebar-top"></div>' + '<div class="sidebar-bottom"></div>'
                    + '</div>' + '<div id="catalog-updown">' + '<div title="向上翻页" id="catalog-up"></div>'
                    + '<div title="向下翻页" id="catalog-down"></div>' + '</div>' + '<div id="catalog">' + '<dl></dl>'
                    + '</div>';
            ...
            this.$wrapper.find('#sidebar').css({
                height : this.settings.height
            });
            this.$wrapper.find('#catalog').css({
                height : this.settings.height - 10
            }); // catalog 有上下margin各15px
            this.$wrapper.find('dl').css({
                width : this.settings.width - 30
            });
            ...
            // }
            // html += '<dd class="'+cataItemClass+'"
            // id="sideToolbar-item-0-'+cata.index+'">'+
            // '<span class="'+cataIndxClass+'">'+cata.index+'</span>'+
            // '<a href="#'+cata.index+'" title="'+cata.title+'" onclick="return
            // false;">'+cata.title+'</a>'+ //return false;
            // '<span class="sideCatalog-dot"></span>'+
            // '</dd>';
            // }

            // 根据 content 内容初始化目录
            heads = this.$content.find('div.headline-1, div.headline-2');
            for (var i = 0; i < heads.length; i++) {
                ...
                html += '<dd class="' + cataItemClass + '" id="cata-headline-' + index + '">' + '<span class=" '
                        + cataIndxClass + '"></span>' + '<a href="#' + index + '" >' + title + '</a>' + // return
                        // false;
                        '<span class="sideCatalog-dot"></span>' + '</dd>';
            }
        ...
        bindEvents : function() {
            ...
            $('.sidebar-top').click(function() {
                context.scrollElement($('#catalog'), 100, 'up', 1); // 100
                // 为自定义的jump长度
            });
        ...
        initCatalogUpDown : function() {
            ...
            if ($('#catalog')[0].scrollHeight > $('#catalog').height()) {
                ...
                $('#catalog')[0].onmousewheel = handler;
        ...
        cataLocationByContent : function() {
            ...
            var cataList = this.$content.find('div.headline-1, div.headline-2');
            var scrollTop = $(document).scrollTop();
            for (var i = 0, len = cataList.length; i < len; i++) {
                var ele = cataList[i], ele1 = cataList[i + 1];
                // 判断当前滚动位置的内容属于哪条目录或子目录
                if ($(ele).offset().top - 30 <= scrollTop
                        && ((i + 1 == len) || ((i + 1 < len) && $(ele1).offset().top > scrollTop))) {
                    this.$wrapper.find('.highlight').removeClass('highlight');
                    var index = ele.getAttribute('id');
                    this.$wrapper.find('[href=#' + index + ']').parent().addClass('highlight');
