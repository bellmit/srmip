...
<body>
  <div class="easyui-layout" data-options="fit:true">
      ...
      <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="tPmPlanDraftController.do?doUpdate" tiptype="1" beforeSubmit="setValue">
        ...
        <table style="width: 800px;" cellpadding="0" cellspacing="1" class="formtable">
          <tr>
            ...
            <td class="value" style="width: 260px;"><input id="planName" name="planName" type="text" style="width: 150px" datatype="*1-50" class="inputxt" value='${tPmPlanDraftPage.planName}' readonly="true">
            ...
            <td class="value" style="width: 260px;"><input id="planTime" name="planTime" type="text" style="width: 150px" class="Wdate" onClick="WdatePicker()" readonly="true"
          ...
          <tr>
            ...
            <td class="value" colspan="3"><textarea id="remark" style="width: 550px;" class="inputxt" rows="6" name="remark" readonly="true">${tPmPlanDraftPage.remark}</textarea> <span class="Validform_checktip"></span>
    ...
    <div data-options="region:'center'">
      <t:datagrid name="tPmProjectList" checkbox="true" fitColumns="false" title="项目列表" actionUrl="tPmPlanDraftController.do?datagridForProject&planId=${tPmPlanDraftPage.id}" idField="id" fit="true"
        queryMode="group" pagination="false">
        <t:dgCol title="主键" field="id" hidden="true" queryMode="group" width="120"></t:dgCol>
        <t:dgCol title="计划草案明细主键" field="planDetailId" hidden="true" ></t:dgCol>
        ...
        <t:dgCol title="责任部门" field="dutyDepart_departname" query="true" isLike="true" queryMode="single" width="120"></t:dgCol>
   		...
   		<t:dgCol title="总经费" field="allFee" queryMode="group" width="120" align="right"></t:dgCol>
...
<%--         <t:dgCol title="总经费" field="allFee" queryMode="group" width="80"></t:dgCol> --%>
        ...
        <c:if test="${show eq '1'}">
        <t:dgCol title="审批状态" field="auditStatus" replace="未审核_0,审核通过_1,审核不通过_2,重新提交_3"  width="90"></t:dgCol>
        <t:dgCol title="操作" field="opt" width="250" frozenColumn="true"></t:dgCol>
        <t:dgFunOpt title="审批通过" exp="auditStatus#ne#1&&auditStatus#ne#2" funname="pass(planDetailId)" ></t:dgFunOpt>
        <t:dgFunOpt title="审批不通过" exp="auditStatus#ne#1&&auditStatus#ne#2" funname="reject(planDetailId)" ></t:dgFunOpt>
        <t:dgFunOpt title="查看修改意见" exp="auditStatus#ne#0&&auditStatus#ne#1" funname="viewMsg(planDetailId)" ></t:dgFunOpt>
        </c:if>
  ...
  <script type="text/javascript">
    ...
    function pass(id){
      $.ajax({
         url:'tPmPlanDraftController.do?doAudit',
         type:'POST',
         dataType:'json',
         data:{'id':id,'opt':'pass'},
    ...
    function reject(id){
        if (typeof (windowapi) == 'undefined') {
            $.dialog({
                id : 'msgTextDialog',
                content : 'url:tPmPlanDraftController.do?goPropose&id=' + id,
                lock : true,
                width : '450px',
                height : '120px',
                title : "填写修改意见",
                ...
                ok:function(){
                    ...
                    $.ajax({
                        ...
                        type:'POST',
                        dataType:'json',
                        data:{'id':id,'opt':'reject','msgText':msgText},
                        success:function(data){
                            tip(data.msg);
                            if(data.success){
                                reloadTable();
