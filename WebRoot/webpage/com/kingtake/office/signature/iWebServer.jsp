...
<%!
  public class iWebRevision {
    ...
    private boolean SaveHistory() {
      ...
      String iID = UUIDGenerator.generate();
    ...
    private boolean SaveSignature() {
      ...
      if (DbaObj.OpenConnection()) {
        ...
        try {
          ...
          OracleResultSet update = (OracleResultSet) stmt.executeQuery("select * from Document_Signature where ID='" + iID+"'");
          if (update.next()) {
        ...
        if (request.getMethod().equalsIgnoreCase("POST")) {
          ...
          if (MsgObj.GetMsgByName("DBSTEP").equalsIgnoreCase("DBSTEP")) {       //检测客户端传递的数据包格式
            ...
            if (mOption.equalsIgnoreCase("SIGNATRUELIST")) {                    //下面的代码为创建印章列表
              ...
              File parentFile = new File(file.getParent()+"/signature/");
              if(!parentFile.exists()){
                  parentFile.mkdirs();
              }
              MsgObj.MsgFileSave(parentFile.getPath()+"/"+mRecordID+"_"+mFieldName+".jpg");    //在服务器保存输出成图片
