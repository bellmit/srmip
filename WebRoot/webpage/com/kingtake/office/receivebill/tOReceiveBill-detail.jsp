...
%>
 ...
 <body onload="load()">
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table"  action="tOReceiveBillController.do?doOperate" tiptype="1">
...
<!-- 			</tr> -->
    ...
    见</p>
					...
					<td class="title3" colspan="5" nowrap="nowrap" height="200px">
                                        <OBJECT name="dutyOption" classid="<%=clsId %>" codebase="<%=codeBase %>" width=100% height=100%>
                                          <param name="WebUrl" value="<%=mServerUrl%>">    <!-- WebUrl:系统服务器路径，与服务器交互操作，如打开签章信息 -->
                                          <param name="RecordID" value="${tOReceiveBillPage.id }">    <!-- RecordID:本文档记录编号 -->
...
<c:if test="${tOReceiveBillPage.createBy eq userName}">
...
<%-- 				<t:tab title="附件" id="ff" href="commonController.do?presentPage"></t:tab> --%>
			</t:tabs>
			</div>
		</div>
</c:if>
