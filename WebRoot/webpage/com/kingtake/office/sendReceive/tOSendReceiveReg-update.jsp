...
// 		  $('#filesId').val("");
  ...
  function initSfdzyp(){
    //初始化是否电子阅批
    var eauditFlag_hidden = $("#eauditFlag_hidden").val();
    if(eauditFlag_hidden=="1"){
        $("#yzbj").attr("checked","checked");
        $("#yzbj").show();
    }else if(eauditFlag_hidden=="0"){
        $("#yzbj").attr("checked","checked");
        $("#yzbj").hide();
    }
  }
...
<body>
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" beforeSubmit="checkFileNum"
    action="tOSendReceiveRegController.do?doUpdate" tiptype="1" callback="@Override uploadFile">
			...
			<tr>
				...
				<td class="value" colspan="5">
					<input id="title" name="title" type="text" style="width: 70%" class="inputxt" 
						value='${tOSendReceiveRegPage.title}' datatype="byterange" max="200" min="1"><c:if test="${tOSendReceiveRegPage.registerType==0}"><t:codeTypeSelect name="type" type="radio" codeType="1" code="SFWLX" id="type" defaultVal="${tOSendReceiveRegPage.type}"></t:codeTypeSelect></c:if>
<%-- 						<t:convert codeType="1" code="SFWLX" value="${tOSendReceiveRegPage.type}"></t:convert> --%>
        ...
        <c:when test="${tOSendReceiveRegPage.registerType==0}">
          <tr>
            ...
            <td class="value" colspan="3"><input id="fileNumPrefix" name="fileNumPrefix" type="text" datatype="*" ignore="ignore"
                ...
                value='${tOSendReceiveRegPage.fileNumYear}'>﹞ <input id="fileNum" name="fileNum" datatype="*" ignore="ignore" type="text"
          ...
          <tr>
            ...
            <td class="value" colspan="3"><input id="filesId" name="filesId" datatype="*1-10" type="text" validType="t_o_send_receive_reg,files_Id,id" errormsg="收文编号重复，请重新获取" style="width: 150px"
      ...
      <c:choose>
          ...
          <tr>
            ...
            <input id="undertakeContactId" name="undertakeContactId" type="hidden" value="${tOSendReceiveRegPage.undertakeContactId}" datatype="byterange" max="36" min="1" nullMsg="承办人不是从用户/下拉列表中选择"> 
            <input id="undertakeContactName" name="undertakeContactName" type="text" style="width: 150px" class="inputxt" value="${tOSendReceiveRegPage.undertakeContactName}"> <t:chooseUser
                icon="icon-search" title="人员列表" textname="id,realName,departId,departName"
            ...
            <td class="value"><input id="undertakeDeptId" name="undertakeDeptId" type="hidden" style="width: 150px" class="inputxt" value="${tOSendReceiveRegPage.undertakeDeptId}"> <input
                id="undertakeDeptName" name="undertakeDeptName" type="text" style="width: 150px" class="inputxt" value="${tOSendReceiveRegPage.undertakeDeptName}" datatype="*"> <span
          ...
          <tr>
            ...
            <td align="right"><label class="Validform_label">是否电子审批: </label></td>
            <td class="value">
               <input id="eauditFlag_hidden" type="hidden" value="${tOSendReceiveRegPage.eauditFlag}">
               <input id="eauditFlagY" name="eauditFlag" type="radio" onclick="ydzyp()" style="width: 20px;height: 20px;" value="1" <c:if test="${tOSendReceiveRegPage.eauditFlag ne '0'}">checked="true"</c:if> >是 
               <input id="eauditFlagN" name="eauditFlag" type="radio" onclick="ndzyp()" style="width: 20px;height: 20px;" value="0" <c:if test="${tOSendReceiveRegPage.eauditFlag eq '0'}">checked="true"</c:if> >否
               <span class="Validform_checktip"></span> <label class="Validform_label" style="display: none;">是否电子审批</label></td>
		 ...
		 <td class="value">
          ...
          <td class="value" colspan="3"><input id="fileNumPrefix" name="fileNumPrefix" type="text"
            ...
            <tr>
              ...
              <td class="value" colspan="3"><textarea id="remark" name="remark" style="width: 87%;" rows="5" class="inputxt" datatype="byterange" max="500" min="0">${tOSendReceiveRegPage.remark}</textarea>
            ...
            <tr>
                ...
                <td class="value" colspan="3"><a class="easyui-linkbutton" data-options="plain:true"  onclick="addMain()" href="#officecontrol" icon="icon-add">添加正文</a></td>
                </c:if>
                <c:if test="${!empty tOSendReceiveRegPage.contentFileId}">
