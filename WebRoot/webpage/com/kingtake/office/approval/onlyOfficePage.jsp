...
// 	$('#officecontrol').attr('style','display:block;height:800px;');
	...
	if(realPath){
		TANGER_OCX_openFromUrl('<%=rootPath%>/upload/template/cpjTH.docx',false);
		taoHong();
...
// 	OFFICE_CONTROL_OBJ.SetReadOnly(false);
	OFFICE_CONTROL_OBJ.SetBookmarkValue("密级","<t:convert codeType="0" code="XMMJ" value="${tOApprovalPage.secrityGrade}"></t:convert>");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("前缀","${tOApprovalPage.fileNumPrefix }");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("年度","${tOApprovalPage.approvalYear }");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("文号","${tOApprovalPage.applicationFileno }");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("承办单位","${tOApprovalPage.undertakeUnitName }");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("联系人","${tOApprovalPage.contactName }");
	OFFICE_CONTROL_OBJ.SetBookmarkValue("电话","${tOApprovalPage.contactPhone }");
	checkFile('/signature/${tOApprovalPage.id }_approvalBillPresident.jpg',"校首长批示");
	checkFile('/signature/${tOApprovalPage.id }_approvalDepartmentLeader.jpg',"部领导批示");
