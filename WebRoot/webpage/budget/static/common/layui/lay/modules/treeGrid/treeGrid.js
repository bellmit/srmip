layui.config({
    ...
    var $ = layui.$
        ...
        ,table = {
            ...
            ,addRow:function (tableId, index, data) {
                ...
                that.resetData(data);
                //插入到父节点后面
                var arr = list.filter(function(item){ return item.PID === list[index - 1].ID});
                list.splice(index + arr.length, 0, data);//更新缓存
                ...
                if(index==0){//在第一个位置插入
                    ...
                    // $(o).after(trs);
                    var o=that.layBody.find('[data-index='+(index-1)+']');//父节点dom树
                    var a = that.layBody.find('tr[u_id="' + o.find('td[data-field="ID"] p').html() + '"]');
                    if(a.length !== 0) {
                        $(a[a.length - 1]).after(trs);
                    } else {
                        $(o).after(trs);
                    }
                }
    ...
    Class.prototype.treeConvertShowName=function (o) {
        ...
        var temhtml='<div style="float: left;height: 28px;line-height: 18px;padding-left: '+
            function () {
                if(isTreeNode){
                    return '5px'
                }else{
