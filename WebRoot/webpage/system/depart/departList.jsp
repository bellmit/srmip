...
<div id="main_depart_list" class="easyui-layout" fit="true">
    <div region="center" style="padding: 1px;">
        ...
        <t:datagrid  name="departList"  title="common.department.list"
            ...
            <t:dgToolBar title="机构编辑"  icon="icon-edit" 
            	url="departController.do?update" width="675" height="530" 
            	funname="update"></t:dgToolBar>
            <%-- <t:dgDelOpt url="departController.do?del&id={id}" title="common.delete" ></t:dgDelOpt> --%>
            <t:dgFunOpt funname="deleteDepart(id)" title="删除"></t:dgFunOpt>
            ...
            <t:dgFunOpt funname="setThesisSecretCode(id)" title="论文保密编号"></t:dgFunOpt>
        </t:datagrid>
    </div>
    </div>
...
function addRow(data){
	if(data.tspdepart){
		var parent = $("#departList").treegrid('find', data.tspdepart.id);
		if(parent){
			var children = $("#departList").treegrid('getChildren', parent.id);
			if(children && children.length > 0){
				$("#departList").treegrid('append', {
					parent : parent.id,
					data : [getTreeNode(data)]
				});
			}
			$("#departList").treegrid('expand', data.tspdepart.id);
		}
	}else{
		var roots = $("#departList").treegrid('getRoots');
		$('#departList').treegrid('insert', {
...
function updateRow(data){
	$("#departList").treegrid('update', {
		id : data.id,
		row : getTreeNode(data)
	});
}
...
function getTreeNode(data){
	return {
		...
		'parentText' : '',
		'fieldMap.shortname' : data.shortname,
		'src' : data.description,
		'fieldMap.orgCode' : data.orgCode,
		'fieldMap.orgType' : data.orgType,
		'fieldMap.subjectDirectionCode' : data.subjectDirectionCode,
		'fieldMap.subjectDirectionName' : data.subjectDirectionName,
		'fieldMap.manager_depart' : data.manager_depart,
		'fieldMap.leader_official' : data.leader_official,
		'order' : data.sortCode,
		'fieldMap.validFlag' : data.validFlag,
		'fieldMap.scientificInstitutionFlag' : data.scientificInstitutionFlag,
	};
}
...
//设置论文保密审查规则编号
function setThesisSecretCode(departid){
    var addurl = "departController.do?goThesisSecretCodePage&departId="+departid;
    if(typeof(windowapi) == 'undefined'){
		$.dialog({
		    ...
		    ok: function(){
		        ...
		        var secretCode = iframe.getSecretCode();
		        if(secretCode!=""){
		           saveSecretCode(departid,secretCode);
		        }
				return false;
		    },
		    cancelVal: '关闭',
...
//保存保密审查编号
function saveSecretCode(departId,secretCode){
    $.ajax({
		type : 'post',
		data : {'departId' : departId,'secretCode':secretCode},
		url : 'departController.do?doSaveSecretCode',
		dataType : 'json',
		success : function(result){
		    iframe.closeDialog();
			tip(result.msg);
