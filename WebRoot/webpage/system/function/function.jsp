...
<body style="overflow-y: hidden" scroll="no">
  <t:formvalid formid="formobj" layout="div" dialog="true" refresh="true" action="functionController.do?saveFunction">
    ...
    <fieldset class="step">
      <div class="form">
        <label class="Validform_label"> <t:mutiLang langKey="menu.name" />:
        </label> <input name="functionName" class="inputxt" value="${function.functionName}" datatype="*2-50"> <span
          class="Validform_checktip"> <t:mutiLang langKey="menuname.rang2to50" />
        </span>
      </div>
      <div class="form">
        <label class="Validform_label"> <t:mutiLang langKey="funcType" />:
      ...
      <div class="form">
        <label class="Validform_label"> <t:mutiLang langKey="menu.level" />:
      ...
      <div class="form" id="pfun">
        <label class="Validform_label"> <t:mutiLang langKey="parent.function" />:
        </label> <input id="cc" <c:if test="${function.TSFunction.functionLevel eq 0}"> value="${function.TSFunction.id}"</c:if>
      ...
      <div class="form" id="funurl">
        <label class="Validform_label"> <t:mutiLang langKey="menu.url" />:
        </label> <input name="functionUrl" class="inputxt" value="${function.functionUrl}">
      </div>
      <div class="form">
        <label class="Validform_label"> <t:mutiLang langKey="common.icon" />:
        </label> <select name="TSIcon.id">
          <c:forEach items="${iconlist}" var="icon">
            <option value="${icon.id}"
              <c:if test="${icon.id==function.TSIcon.id || (function.id eq null && icon.iconClas eq 'default') }">selected="selected"</c:if>>
      ...
      <div class="form">
        <label class="Validform_label"> <t:mutiLang langKey="desktop.icon" />:
        </label> <select name="TSIconDesk.id">
          <c:forEach items="${iconDeskList}" var="icon">
            <option value="${icon.id}"
              <c:if test="${icon.id==function.TSIconDesk.id || (function.id eq null && icon.iconClas eq 'System Folder') }">selected="selected"</c:if>>
              <t:mutiLang langKey="${icon.iconName}" />
            </option>
          </c:forEach>
        </select>
      ...
      <div class="form" id="businessCode">
        <label class="Validform_label"> 业务编码:
        </label> <input name="businessCode" class="inputxt" value="${function.businessCode}" datatype="s2-32" ignore="ignore">
      </div>
      <div class="form" id="funorder">
        <label class="Validform_label"> <t:mutiLang langKey="menu.order" />:
        </label> <input name="functionOrder" class="inputxt" value="${function.functionOrder}" datatype="n1-3">
      </div>
    </fieldset>
  </t:formvalid>
</body>
