...
function Login(orgId) {
	...
	//var password = $("#password").val();
	if(!username||username=="用户名 Username"){
		msg+="用户名";
	}
	if(!password||password=="密码 Password"){
		msg+="密码";
	}
	
	if(msg){showError("请输入"+msg); return;}
...
function setCookie()
{
	if ($('#on_off').val() == '1') {
		$("input[iscookie='true']").each(function() {
			$.cookie(this.name, $("#"+this.name).val(), {path:"/",expires:60});
			$.cookie("COOKIE_NAME","true", {path:"/",expires:60});
		});
	} else {
		$("input[iscookie='true']").each(function() {
...
//            update-end--Author:zhangguoming  Date:20140429 for：是否记住用户名优化
		...
		$(".fn-rememberaccout").addClass("rememberaccout-active");
		$("#on_off").val("1");
	} 
	else
	{
		//$("#on_off").attr("checked", false);
		$(".fn-rememberaccout").removeClass("rememberaccout-active");
		$("#on_off").val("0");
//      update-begin--Author:zhangguoming  Date:20140429 for：是否记住用户名优化
        $("#randCode").focus();
//      update-end--Author:zhangguoming  Date:20140429 for：是否记住用户名优化
