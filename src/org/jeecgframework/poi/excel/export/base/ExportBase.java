...
public class ExportBase {
    ...
    private void getExcelField(String targetId, Field field, ExcelExportEntity excelEntity, Excel excel,
        ...
        excelEntity.setExportConvert(excel.isExportConvert());
        excelEntity.setAmount(excel.isAmount());
        if (excel.isExportConvert()) {
            excelEntity.setConvertMethod(POIPublicUtil.getConvertMethod(fieldname, pojoClass));
        }
    }
        ...
        for (int i = 0; i < excelParams.size(); i++) {
            ...
            if (excelParams.get(i).getList() != null) {
                for (int j = 0; j < excelParams.get(i).getList().size(); j++) {
                    maxHeight = maxHeight > excelParams.get(i).getList().get(j).getHeight() ? maxHeight : excelParams
                            .get(i).getList().get(j).getHeight();
                }
            }
        }
    ...
    public Object getCellValue(ExcelExportEntity entity, Object obj) throws Exception {
        Object value = entity.getMethods() != null ? getFieldBySomeMethod(entity.getMethods(), obj) : entity
        ...
        if (value != null && entity.isExportConvert() && entity.getConvertMethod() != null) {
            value = entity.getConvertMethod().invoke(obj, new Object[] {});
        }
        if (value != null && entity.isAmount()) {//如果是金额，需要进行转化
            value = oConvertUtils.formatAmount(value);
        }
