...
public class ParseWord07 {
     ...
     * @date 2013-11-16
     */
    private void changeValues(XWPFParagraph paragraph, XWPFRun currentRun, String currentText, List<Integer> runIndex,
        ...
        if (obj instanceof WordImageEntity) {// 如果是图片就设置为图片
            ...
            if (obj.toString().indexOf("\n") != -1) {
                ...
                for (int i = 0; i < s.length; i++) {
                    String text = s[i];
                    currentRun.setText(text.trim(), i);
                    if (i < s.length - 1) {
                        currentRun.addBreak();
                    }
                }
    ...
    private void addAnImage(WordImageEntity obj, XWPFRun currentRun) throws Exception {
        ...
        try {
            ...
            ((JeecgXWPFDocument) currentRun.getParagraph().getDocument()).createPicture(currentRun, picId, currentRun
                    .getParagraph().getDocument().getNextPicNameNumber((Integer) isAndType[1]), obj.getWidth(),
                    obj.getHeight());
