...
public class TaskJeecgService{
    ...
    private void turnTransition(String taskId, String activityId, Map<String, Object> variables) throws Exception {
        ...
        List<PvmTransition> oriPvmTransitionList = clearTransition(currActivity);//清空当前节点的出口，并存储起来

        TransitionImpl newTransition = currActivity.createOutgoingTransition();//创建一个新出口

        ActivityImpl pointActivity = findActivitiImpl(taskId, activityId);//查找目标节点

        newTransition.setDestination(pointActivity);//设置出口的目的地
        ...
        for (PvmTransition pv : oriPvmTransitionList) {
            if (activityId.equals(pv.getDestination().getId())) {
                ...
                if (tra.getExecutionListeners() != null) {
                    newTransition.setExecutionListeners(tra.getExecutionListeners());
                }
                break;
            }
        }
        this.taskService.complete(taskId, variables);//完成任务节点

        pointActivity.getIncomingTransitions().remove(newTransition);

        restoreTransition(currActivity, oriPvmTransitionList);//重建分支
    }
     ...
     * 修改流程状态
     */
    public void updateFormDataStatus(DelegateExecution execution, String bpmStatus) {
    	//将业务数据的流程状态改为已完成
		Object bpm_form_key = execution.getVariable(WorkFlowGlobals.BPM_FORM_KEY);//获取流程对应的表单
		if(oConvertUtils.isNotEmpty(bpm_form_key)){
			//修改业务数据的状态
            Object id = execution.getVariable(WorkFlowGlobals.BPM_DATA_ID);//获取流程对应的表单
            activitiDao.updateFormDataStatus((String) id, (String) bpm_form_key, bpmStatus);

            String sql = "update t_s_basebus t set t.PRJSTATEID=? where t.id=?";
            SystemService systemService = ApplicationContextUtil.getContext().getBean(SystemService.class);
            systemService.executeSql(sql, new Object[] { bpmStatus, id });
		}
    }
