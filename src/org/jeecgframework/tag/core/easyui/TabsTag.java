...
public class TabsTag extends TagSupport {
    ...
    public StringBuffer end() {
        ...
        if (iframe) {
            ...
            if (tabList.size() > 0) {
                for (int i = 0;i<tabList.size();i++) {
                    ...
                    sb.append("add" + id + "(\'" + tab.getTitle() + "\',\'" + tab.getHref() + "\',\'" + tab.getId()
                            + "\',\'" + tab.getIcon() + "\',\'" + tab.isClosable() + "\',"+selected+");");
                }
            }
            sb.append("function add" + id + "(title,url,id,icon,closable,selected) {");
            ...
            sb.append("{");
            sb.append("var s = \'<iframe id=\"\'+id+\'\" scrolling=\"yes\" frameborder=\"0\"  src=\"about:jeecg\" width=\""
                    + oConvertUtils.getString(width, "100%")
                    + "\" height=\""
                    + oConvertUtils.getString(heigth, "99.5%") + "\"></iframe>\';");
            sb.append("return s;");
            sb.append("}");
            sb.append("});");
            sb.append("</script>");
        ...
        if (tabs) {
            ...
            sb.append("<div id=\"" + id + "\" tabPosition=\"" + tabPosition
                    ...
                    + oConvertUtils.getString(width, "auto") + ";\"");
            if (StringUtils.isNotEmpty(tools)) {
                sb.append(" data-options=\"tools:'" + tools + "'\" ");
            }
            sb.append(" class=\"easyui-tabs\" fit=\"" + fit + "\">");
            if (!iframe) {
                for (Tab tab : tabList) {
                    if (tab.getHref() != null) {
                        sb.append("<div title=\""
                        ...
                        sb.append("<div title=\"" + tab.getTitle()
                                + "\"  style=\"margin:0px;padding:0px;overflow-x:hidden;overflow-y:auto;width=auto;\">");
                        sb.append("<iframe id=\"" + tab.getId() + "\" scrolling=\"yes\" frameborder=\"0\"  src=\""
                                + tab.getIframe() + "\" width=\"" + oConvertUtils.getString(tab.getWidth(), "100%")
    ...
    public void setTab(String id, String title, String iframe, String href, String iconCls, boolean cache,
            String content, String width, String heigth, boolean closable) {
        Tab tab = new Tab();
        tab.setId(id);
        tab.setTitle(title);
        tab.setHref(href);
