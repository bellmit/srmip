...
public class UploadTag extends TagSupport {
    ...
    protected String onQueueComplete;
    protected boolean view = false;// 生成查看删除链接
    protected String methodPrefix = "";// upload和cancel方法前缀，若指定，则方法为methodPrefix+upload和methodPrefix+cancel，
    private String fileSizeLimit = "500MB";//上传文件最大限制
    ...
    public StringBuffer end() {
        ...
        sb.append("<script type=\"text/javascript\">" + "var flag = false;" + "var fileitem=\"\";"
                + "var fileKey=\"\";" + "var serverMsg=\"\";" + "var m = new Map();" + "$(function(){" + "$(\'#" + id
                + "\').uploadify({" + "buttonText:\'" + buttonText + "\'," + "auto:" + auto + ","
                + "progressData:\'speed\'," + "multi:" + multi + ",");
        if (!multi) {
            sb.append("uploadLimit:1,");
            sb.append("queueSizeLimit:1,");
        }
        sb.append("height:25," + "overrideEvents:[\'onDialogClose\']," + "fileTypeDesc:\'文件格式:\'," + "queueID:\'"
                + queueID + "\'," + "fileTypeExts:\'" + extend + "\'," + "fileSizeLimit:\'"+fileSizeLimit+"\',"
                + "swf:\'plug-in/uploadify/uploadify.swf\',	" + "uploader:\'" + getUploader()
        ...
        ;
        sb.append("} ," );
        sb.append("onQueueComplete :");
        
        if(onQueueComplete != null) {
        	 sb.append(onQueueComplete).append(",");
        }else {
	        sb.append("function(queueData) { ");
	        if (dialog) {
	            sb.append("var win = frameElement.api.opener;" + "win.reloadTable();" + "win.tip(serverMsg);"
...
//        sb.append("var win = frameElement.api.opener;");
        ...
        sb.append("},"
                ...
                + "}" + "});" + "});" + "function " + methodPrefix + "upload() {" + "	$(\'#" + id
                + "\').uploadify('upload', '*');" + "		return flag;" + "}" + "function " + methodPrefix + "cancel() {"
                + "$(\'#" + id + "\').uploadify('cancel', '*');" + "}" + "</script>");
