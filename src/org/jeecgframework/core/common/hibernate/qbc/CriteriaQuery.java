...
/**
 * 
 * 类描述：CriteriaQuery类是对hibernate QBC查询方法的封装，需要的参数是当前操作的实体类 张代浩
 * 
...
public class CriteriaQuery implements Serializable {
    ...
    private int total;
    private final List<String> alias = new ArrayList<String>();// 保存创建的aliasName 防止重复创建
    ...
    /**
     * 加载条件(条件之间有关联) 
     * 1)hql1((this_.0 like ? and this_.1 like ?) or this_.2 like ?)
     *   表示法cq.add(cq.or(cq.and(cq, 0, 1), cq,
     * 2)hql2:(this_.0 like ? or this_.1 like ?) 
     *   表示法:cq.add(cq.or(cq, 0, 1)); 
     ...
     * @param source
     * @param dest
     *            hql((this_.0 like ? and this_.1 like ?) or this_.2 like ?) 表示法cq.add(cq.or(cq.and(cq, 0, 1), cq, 2))
     * @return
     */
     ...
     * @param source
     * @param dest
     *            hql:(this_.0 like ? or this_.1 like ?) 表示法:cq.add(cq.or(cq, 0, 1));
     * @return
     */
     ...
     * @param keyvalue1
     * @param keyvalue2
     *            两个条件或查询： Restrictions.or(Restrictions.in("username",list1), Restrictions.idEq(1)); 三个或多个条件查询:（使用嵌套方式）
     *            criteria.add(Restrictions .or(Restrictions.in("username",list1), Restrictions.or(Restrictions.idEq(3),
     *            Restrictions.idEq(4))));
     */
    ...
    public void in(String keyname, Object[] keyvalue) {
        if (keyvalue != null && keyvalue[0] != "") {
            if (keyvalue.length <= 1000) {
                ...
                List<List<Object>> sortList = sortList(keyvalue);
                Disjunction disjunction = Restrictions.disjunction();
                for (List<Object> list : sortList) {
                    disjunction.add(Restrictions.or(Restrictions.in(keyname, list.toArray())));
                }
                criterionList.addPara(disjunction);
            }
        }
    }
     ...
     * @return
     */
    private List<List<Object>> sortList(Object[] keyvalue) {
        List<List<Object>> sortList = new ArrayList<List<Object>>();
        ...
        for (int i = 0; i < sort; i++) {
            ...
            for (int j = 0; j < 1000; j++) {
                tmpList.add(keyvalue[i * 1000 + j]);
            }
            sortList.add(tmpList);
        }
        if (end != 0) {//最后一组
        List<Object> endList = new ArrayList<Object>();
        for (int w = sort * 1000; w < keyvalue.length; w++) {
            endList.add(keyvalue[w]);
        }
        sortList.add(endList);
        }
        return sortList;
    }
